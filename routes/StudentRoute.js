...
route.get('/getallfriend',async (rq, rs) => {
    try {
        ...
        rs.json({Message:"Student Fetched",data:friend})
    }
    catch (e) {
        rs.json({ message: e.message })
    }
...
route.get('/studentdata/:sid',async(rq,rs)=>{
    try{
        const {sid} = rq.params
        const student = await friendModel.findById(sid).populate("course")
        rs.json({Message : "Student Details",Student:student})
    }
    catch(e){
        rs.json({Message:e.message})
    }
